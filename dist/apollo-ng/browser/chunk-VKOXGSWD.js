  ...
  if (a(e)) {
    ...
    if (G(a2, k3)) {
      let n2 = a2.replaceAll(k3, (l3) => {
        let c4 = l3.replace(/{|}/g, "").split(".").filter((m2) => !s4.some((d3) => G(m2, d3)));
...
var w4 = (e = {}) => {
  ...
  return { preset(s4) {
    ...
    var o, c4;
    let { semantic: i3 } = t2 || {}, a2 = s4 && Object.hasOwn(s4, "light") ? s4.light : s4, n2 = s4 && Object.hasOwn(s4, "dark") ? s4.dark : s4, l3 = { colorScheme: { light: h2(h2({}, (o = i3 == null ? void 0 : i3.colorScheme) == null ? void 0 : o.light), !!a2 && { surface: a2 }), dark: h2(h2({}, (c4 = i3 == null ? void 0 : i3.colorScheme) == null ? void 0 : c4.dark), !!n2 && { surface: n2 }) } };
...
function de(e, t2 = {}) {
  ...
  for (; o.length; ) {
    ...
    for (let u2 in m2) {
      let g2 = m2[u2], f2 = ve(g2), p3 = G(u2, a2) ? Q(d3) : Q(d3, re(u2));
...
var BaseStyle = class _BaseStyle {
  ...
  classes = {};
  inlineStyles = {};
  load = (style17, options = {}, transform = (cs) => cs) => {
    const computedStyle = transform(ar`${m(style17, {
      dt: E2
    })}`);
  ...
  loadStyle = (options = {}, style17 = "") => {
    return this.load(this.style, options, (computedStyle = "") => S2.transformCSS(options.name || this.name, `${computedStyle}${ar`${style17}`}`));
  };
  loadBaseCSS = (options = {}) => {
...
var DomHandler = class _DomHandler {
  ...
  static getOuterWidth(el, margin) {
    ...
    if (margin) {
      let style17 = getComputedStyle(el);
      width += parseFloat(style17.marginLeft) + parseFloat(style17.marginRight);
    }
    return width;
  }
  static getHorizontalPadding(el) {
    let style17 = getComputedStyle(el);
    return parseFloat(style17.paddingLeft) + parseFloat(style17.paddingRight);
  }
  static getHorizontalMargin(el) {
    let style17 = getComputedStyle(el);
    return parseFloat(style17.marginLeft) + parseFloat(style17.marginRight);
  }
  static innerWidth(el) {
    let width = el.offsetWidth;
    let style17 = getComputedStyle(el);
    width += parseFloat(style17.paddingLeft) + parseFloat(style17.paddingRight);
    return width;
  }
  static width(el) {
    let width = el.offsetWidth;
  ...
  static getInnerHeight(el) {
    ...
    let style17 = getComputedStyle(el);
    height += parseFloat(style17.paddingTop) + parseFloat(style17.paddingBottom);
    return height;
  }
  static getOuterHeight(el, margin) {
    ...
    if (margin) {
      let style17 = getComputedStyle(el);
      height += parseFloat(style17.marginTop) + parseFloat(style17.marginBottom);
    }
    return height;
  }
  static getHeight(el) {
    let height = el.offsetHeight;
    let style17 = getComputedStyle(el);
    height -= parseFloat(style17.paddingTop) + parseFloat(style17.paddingBottom) + parseFloat(style17.borderTopWidth) + parseFloat(style17.borderBottomWidth);
    return height;
  }
  static getWidth(el) {
    let width = el.offsetWidth;
    let style17 = getComputedStyle(el);
    width -= parseFloat(style17.paddingLeft) + parseFloat(style17.paddingRight) + parseFloat(style17.borderLeftWidth) + parseFloat(style17.borderRightWidth);
    return width;
  }
  static getViewport() {
  ...
  static calculateScrollbarWidth(el) {
    if (el) {
      let style17 = getComputedStyle(el);
      return el.offsetWidth - el.clientWidth - parseFloat(style17.borderLeftWidth) - parseFloat(style17.borderRightWidth);
    } else {
...
var SelectButton = class _SelectButton extends BaseEditableHolder {
  ...
  changeTabIndexes(event, direction) {
    ...
    for (let i3 = 0; i3 <= this.el.nativeElement.children.length - 1; i3++) {
      if (this.el.nativeElement.children[i3].getAttribute("tabindex") === "0") firstTabableChild = {
        elem: this.el.nativeElement.children[i3],
        index: i3
      };
    }
    if (direction === "prev") {
...
var RadioControlRegistry2 = class _RadioControlRegistry {
  ...
  remove(accessor) {
    this.accessors = this.accessors.filter((c4) => {
      return c4[1] !== accessor;
    });
  }
  select(accessor) {
    this.accessors.forEach((c4) => {
      if (this.isSameGroup(c4, accessor) && c4[1] !== accessor) {
        c4[1].writeValue(accessor.value);
      }
    });
  }
...
var RadioButton = class _RadioButton extends BaseEditableHolder {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({
    ...
    features: [\u0275\u0275ProvidersFeature([RADIO_VALUE_ACCESSOR2, RadioButtonStyle, {
...
@angular/animations/fesm2022/animations.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-VKOXGSWD.js.map
